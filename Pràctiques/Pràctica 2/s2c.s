...
moda:
	...
	li $t1, 10
	move $t2, $sp			# Adreça inici de histo[] és $sp
...
for_2:	bge $s2, $s1, fi_2		# Comprovem si es dona la condició de fi del for
	...
	add $t0, $s0, $s2
	lb $t0, 0($t0)
	addi $a1, $t0, -48		# Nota: '0' == 48 (en decimal)
	addi $a2, $s3, -48
	
	jal	update
	
	addi $s3, $v0, '0'		# max += update(...)
	addi $s2, $s2, 1
	b for_2
...
update:
	...
	sll $t0, $s1, 2			
	add $t0, $s0, $t0		# Obtenim adreça h[i] a $t0
	lw $t1, 0($t0)			# $t1 = h[i]
	addi $t1, $t1, 1		# $t1 += 1
	sw $t1, 0($t0)			# h[i] = $t1
